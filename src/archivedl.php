...
while ( ! empty ( $to_download ) ) {
	...
	echo 'avant file_force_contents file='.$file;
...
function relative_to_absolute(string $relative, string $source): string {
	...
	echo 'relative_to_absolute relative='.$relative.'\n';
    echo 'relative_to_absolute source='.$source.'\n';
	...
	echo 'relative_to_absolute ret='.$ret.'\n';
	return $ret;
}
		 ...
		 mkdir ( $tmp . DIRECTORY_SEPARATOR, 0777, TRUE );
			try {
			 mkdir ( $tmp . DIRECTORY_SEPARATOR, 0777, TRUE );
				...
				echo '\njd:Exception reçue : ',$tmp ,  $ex->getMessage(), "\n";
			}
		}
	}
...
class hhb_curl {
	...
	public function _replaceCurl($newcurl, bool $closeold = true) {
		if (! is_resource ( $newcurl )) {
			throw new Exception ( 'parameter 1 must be a curl resource!' );
		}
